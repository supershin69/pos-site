...
@section('content')
    <div class="mt-3 col">
        ...
        <div class="mb-4 d-sm-flex align-items-center justify-content-between">
            <a href="{{ route('admin#list') }}"><button class="text-white btn btn-secondary">Admins</button></a>
        ...
        <table class="table shadow-sm table-hover ">
            ...
            <tbody>
                @foreach ($users as $user)
                    <tr>
                        <td>{{ $user->id }}</td>
                        <td><a href="{{ route('user#peek', $user->id) }}">{{ $user->name }}</a></td>
                        <td><span class="p-2 text-white badge bg-primary rounded-pill text-uppercase">
                                {{ $user->role }}
                            </span></td>
                        <td> {{ $user->created_at->format('d-F-Y (h:m:s)') }} </td>
                        <td>
